...
render_framework::vk_parameters_list_impl::vk_parameters_list_impl(
...
void render_framework::vk_parameters_list_impl::on_parameter_updated(const parameter* p, const uint8_t* value)
{
...
render_framework::vk_uniform_parameters_list_impl::vk_uniform_parameters_list_impl(
...
void render_framework::vk_uniform_parameters_list_impl::on_parameter_updated(const parameter* parameter, const uint8_t* value)
{
...
void render_framework::vk_uniform_parameters_list_impl::flush(const vk_parameters_list_flush_data_source* flush_data_source)
{
    if (!m_dirty) {
        return;
    }
    ...
    vkCmdCopyBuffer(flush_data_source->get_command_buffer(), m_staging_buffer, m_uniform_buffer, 1, &buffer_copy);

    m_dirty = false;
}


render_framework::vk_push_constant_parameters_list_impl::vk_push_constant_parameters_list_impl(
    std::vector<parameter_data>& parameters, 
    std::unordered_map<const parameter*, size_t>& parameters_map, 
    size_t buffer_size)
    : vk_parameters_list_impl(parameters, parameters_map, buffer_size)
...
void render_framework::vk_push_constant_parameters_list_impl::flush(const vk_parameters_list_flush_data_source* flush_data_source)
{
    const auto* flush_data_source_impl = static_cast<const vk_push_constant_parameters_list_flush_data_source*>(flush_data_source);

    vkCmdPushConstants(
      flush_data_source->get_command_buffer(), 
      flush_data_source_impl->get_pipeline_layout(), 
      flush_data_source_impl->get_shader_stages(), 
      0, 
      m_data_buffer.size(),
      m_data_buffer.data());
}
...
ERROR_TYPE render_framework::vk_uniform_parameters_list_builder::create(parameters_list& reslut)
{
    ...
    PASS_ERROR(vk_utils::create_buffer(
        uniform_buffer, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY, args.buffer_size, NULL));
    PASS_ERROR(vk_utils::create_buffer(
        uniform_staging_buffer, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU, args.buffer_size, NULL));


    reslut = parameters_list::create<vk_uniform_parameters_list_impl>(
...
ERROR_TYPE render_framework::vk_push_constant_parameters_list_builder::create(parameters_list& reslut)
{
    parameters_list_args args{};
    PASS_ERROR(get_paramters_list_args(args));

    reslut = parameters_list::create<vk_push_constant_parameters_list_impl>(args.params_data, args.parameters_map, args.buffer_size);

    RAISE_ERROR_OK();
}

ERROR_TYPE render_framework::vk_parameters_list_builder::get_paramters_list_args(parameters_list_args& args)
{
    ...
    for (const auto& param : m_parameters_list) {
        ...
        vk_uniform_parameters_list_impl::parameter_data curr_data{
