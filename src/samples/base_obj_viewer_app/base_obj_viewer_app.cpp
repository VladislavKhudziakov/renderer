...
base_obj_viewer_app::base_obj_viewer_app(const char* app_name, int argc, const char** argv)
...
ERROR_TYPE base_obj_viewer_app::on_vulkan_initialized()
{
    ...
    for (size_t i = 2; i < m_app_info.argc; i++) {
        auto argv = m_app_info.argv[i];
        if (auto rtech = strstr(argv, "--render_technique="); rtech != nullptr) {
            rtech += strlen("--render_technique=");
            if (strcmp(rtech, "PBR")) {
                RAISE_ERROR_FATAL(-1, "PBR currently unsupported.");
            } else if (strcmp(rtech, "PHONG")) {
                model_info.model_render_technique = vk_utils::obj_loader::PHONG;
            } else {
        ...
        if (auto subgeom = strstr(argv, "--subgeometry_textures="); subgeom != nullptr) {
            ...
            std::array<std::string, vk_utils::obj_loader::PHONG_SIZE>& phong_textures = model_info.phong_textures[subgeom_name];
            std::array<std::string, vk_utils::obj_loader::PBR_SIZE>& pbr_textures = model_info.pbr_textures[subgeom_name];
    ...
    PASS_ERROR(loader.load_model(m_app_info.argv[1], model_info, vk_utils::context::get().queue(vk_utils::context::QUEUE_TYPE_GRAPHICS), m_command_pool, m_model));
