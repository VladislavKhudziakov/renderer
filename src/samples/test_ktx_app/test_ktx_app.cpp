...
namespace
{
    struct test_ktx_args_parser : public base_obj_viewer_app::args_parser
    {
        test_ktx_args_parser()
            : base_obj_viewer_app::args_parser()
        {
            m_parse_functions.push_back([this](const char* curr_arg) {
                if (auto s = strstr(curr_arg, "--skybox_tex="); s != nullptr) {
                    auto len = strlen(curr_arg);
                    s += strlen("--skybox_tex=");
                    while ((s != curr_arg + len) && (*s == ' ' || *s == '\n')) {
                        s++;
                    }

                    m_skybox_tex = s;
                    m_model_info.other_textures.emplace_back(m_skybox_tex);
                }
            });
        }

        ~test_ktx_args_parser() = default;

        const std::string& get_skybox_tex()
        {
            return m_skybox_tex;
        }

        private:
            std::string m_skybox_tex;
    };
}

test_ktx_app::test_ktx_app(const char* app_name)
    : base_obj_viewer_app(app_name, std::make_unique<test_ktx_args_parser>())
{
}

ERROR_TYPE test_ktx_app::on_vulkan_initialized()
{
...
ERROR_TYPE test_ktx_app::on_swapchain_recreated()
{
    PASS_ERROR(init_framebuffers());
    PASS_ERROR(record_command_buffers());
...
ERROR_TYPE test_ktx_app::draw_frame()
{
...
ERROR_TYPE test_ktx_app::init_render_passes(){
...
ERROR_TYPE test_ktx_app::init_framebuffers()
{ 
...
ERROR_TYPE test_ktx_app::init_shaders()
{
    ...
    PASS_ERROR(vk_utils::load_shader(WORK_DIR"/shaders/test_ktx.vert.spv", vs, vs_bit));
    PASS_ERROR(vk_utils::load_shader(WORK_DIR"/shaders/test_ktx.frag.spv", fs, fs_bit));
    ...
    VkDescriptorSetLayoutCreateInfo desc_set_layout_infos[]{
        {
    ...
    if (desc_set_layout.init(vk_utils::context::get().device(), desc_set_layout_infos) != VK_SUCCESS) {
    ...
    VkDescriptorPoolCreateInfo desc_pool_info{
        ...
        .maxSets = 1,
        .poolSizeCount = std::size(desc_pool_sizes),
    ...
    {
        .sampler = m_model.textures[0].sampler,
        .imageView = m_model.textures[0].image_view,
        .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
    };

    VkWriteDescriptorSet write_ops[]{
        {
    ...
    vkUpdateDescriptorSets(vk_utils::context::get().device(), std::size(write_ops), write_ops, 0, nullptr);
...
ERROR_TYPE test_ktx_app::init_pipelines()
{ 
...
ERROR_TYPE test_ktx_app::record_command_buffers()
{
