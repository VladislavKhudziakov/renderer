...
ERROR_TYPE dummy_obj_viewer_app::on_swapchain_recreated()
{
...
ERROR_TYPE dummy_obj_viewer_app::on_window_size_changed(int w, int h)
{
...
ERROR_TYPE dummy_obj_viewer_app::init_dummy_shaders(
...
ERROR_TYPE dummy_obj_viewer_app::init_main_render_pass()
{
...
ERROR_TYPE dummy_obj_viewer_app::init_main_frame_buffers()
{
    ...
    if (m_main_depth_image.init(vk_utils::context::get().allocator(), &img_info, &img_alloc_info) != VK_SUCCESS) {
    ...
    if (m_main_msaa_image.init(vk_utils::context::get().allocator(), &img_info, &img_alloc_info)) {
        RAISE_ERROR_FATAL(-1, "cannot init msaa image.");
    }
    ...
    if (m_main_depth_image_view.init(vk_utils::context::get().device(), &img_view_info) != VK_SUCCESS) {
    ...
    if (m_main_msaa_image_view.init(vk_utils::context::get().device(), &img_view_info)) {
        RAISE_ERROR_FATAL(-1, "cannot init msaa image view.");
    }
